...
public class EL002_1_BOM_Check implements IEventAction
{
	...
	static boolean EL002(IChange change)
	{
		...
		try{
			...
			ArrayList<String> affitemlist = new ArrayList();
			...
			while(it.hasNext()){
				...
				affitemlist.add(item.getName());
				log.log(item.getName()+"-------AFF");
			...
			for(IItem item : items)
			{
				...
				for(IRow bom : BOMitems)
				{
					...
					{
						log.log(bom.getCell(ItemConstants.ATT_BOM_ITEM_NUMBER)+"-------BOM");
						if(affitemlist.contains(bom.getCell(ItemConstants.ATT_BOM_ITEM_NUMBER).toString())){
							log.log(2,bom.getCell(ItemConstants.ATT_BOM_ITEM_NUMBER)+": 為AFFECTEDITEMS，故正確");
						}else{
	...
	public EventActionResult doAction(IAgileSession arg0, INode arg1, IEventInfo req) {
		...
		try {
//			log.setLogFile("C:\\Users\\anselm\\BOM.txt");
			log.log("開始執行EL002");
			...
			String status = change.getValue(ChangeConstants.ATT_COVER_PAGE_STATUS).toString();
			IAdmin m_admin = arg0.getAdminInstance();
			...
			log.log(status);
			log.log(1,status.equalsIgnoreCase("Build BOM")||status.equalsIgnoreCase("RD Create"));
			if(status.equalsIgnoreCase("Build BOM")||status.equalsIgnoreCase("RD Create")){
				ret = EL002(change);
			}else{
				return new EventActionResult(req, new ActionResult(ActionResult.STRING, "非檢查站別"));
			}
			log.log("END");
			if(ret == false)
				throw new Exception();
			return new EventActionResult(req, new ActionResult(ActionResult.STRING, "EL002_1程式結束，結果: 成功"));
		} catch (Exception e) {
			...
			try {
				...
				// TODO Auto-generated catch block
				return new EventActionResult(req, new ActionResult(ActionResult.EXCEPTION, new Exception("SET VALUE FAIL")));
			}
			return new EventActionResult(req, new ActionResult(ActionResult.EXCEPTION, new Exception("有錯誤訊息，請至Cover Page Change Order Information 查詢")));
		}
